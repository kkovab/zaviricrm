...
export function packagePricing(activeListings) {
  ...
  const suggestedListingMin = Math.ceil(listingCount * 1.05);
  const suggestedListingMax = Math.ceil(listingCount * 1.1);
  const minOfferPackage =
    PRICING_PACKAGES.find(({ capacity }) => suggestedListingMin <= capacity) ||
    PRICING_PACKAGES[PRICING_PACKAGES.length - 1];
  const maxOfferPackage =
    PRICING_PACKAGES.find(({ capacity }) => suggestedListingMax <= capacity) ||
    PRICING_PACKAGES[PRICING_PACKAGES.length - 1];
  ...
  return {
    ...
    suggested_listing_min: suggestedListingMin,
    suggested_listing_max: suggestedListingMax,
    suggested_price_per_listing_min: minOfferPackage.pricePerListing,
    suggested_price_per_listing_max: maxOfferPackage.pricePerListing,
    suggested_monthly_min: Number((suggestedListingMin * minOfferPackage.pricePerListing).toFixed(2)),
    suggested_monthly_max: Number((suggestedListingMax * maxOfferPackage.pricePerListing).toFixed(2)),
  };
}
...
export function withPackagePricing(agency) {
  ...
  return pricing
    ...
    : {
        ...
        est_monthly_value: 0,
        suggested_listing_min: null,
        suggested_listing_max: null,
        suggested_price_per_listing_min: null,
        suggested_price_per_listing_max: null,
        suggested_monthly_min: null,
        suggested_monthly_max: null,
      };
}
